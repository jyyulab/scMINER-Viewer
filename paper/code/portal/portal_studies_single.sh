#!/usr/bin/env bash
# paper/code/portal/portal_studies_single.sh
#
...
#
# Usage:
#   ./paper/code/portal/portal_studies_single.sh --configs-dir paper/configs
#   ./paper/code/portal/portal_studies_single.sh --configs-dir <path> --mem 64000 --wall 12:00
#   ./paper/code/portal/portal_studies_single.sh --configs-dir <path> --only 2327,2326
#   ./paper/code/portal/portal_studies_single.sh --studies-root /research/.../studies
#
...
mkdir -p paper/metrics/portal_studies paper/logs

STAMP="$(date +%Y%m%d_%H%M%S)"

# LSF's `-env "all, K=V"` form treats commas as separators, so a value
# like ONLY=2317,2333 trips "Invalid syntax for option -env." Some LSF
# versions accept `\,` as an escape, others don't.
#
# Robust workaround: export the values into the submitter's shell env
# and pass `-env "all"` -- LSF forwards the whole environment verbatim,
# never having to parse value contents.
CONFIGS_ABS=""
STUDIES_ABS=""
if [[ -n "$CONFIGS_DIR" ]]; then
  CONFIGS_ABS="$(cd "$CONFIGS_DIR" && pwd)"
  export CONFIGS_DIR="$CONFIGS_ABS"
fi
if [[ -n "$STUDIES_ROOT" ]]; then
  STUDIES_ABS="$(cd "$STUDIES_ROOT" && pwd)"
  export STUDIES_ROOT="$STUDIES_ABS"
fi
if [[ -n "$ONLY" ]]; then
  export ONLY="$ONLY"
fi
...
if [[ -n "$ONLY" ]]; then
  ...
  OUT_TSV="paper/metrics/portal_studies/portal_studies_${ONLY_TAG}.tsv"
else
  OUT_TSV="paper/metrics/portal_studies/portal_studies.tsv"
fi
export OUT_TSV
ENV_STR="all"
...
if [[ "$DRY_RUN" == "1" ]]; then
  echo "[dry-run]:"
  printf '    %q ' "${BSUB_CMD[@]}"; echo "< paper/code/portal/portal_studies.bsub"
  exit 0
fi

"${BSUB_CMD[@]}" < paper/code/portal/portal_studies.bsub

#!/usr/bin/env Rscript
# paper/code/benchmarks/tables.R
#
# Generate publication-ready tables from the benchmark TSVs:
#
#   paper/metrics/benchmarks/bundle_scaling.tsv      (synthetic sweep -- figure 1)
#   paper/metrics/portal_studies/portal_studies.tsv      (real 26+ portal studies)
#
# Writes for each:
...
load_portal_metrics <- function(metrics_dir = "paper/metrics/portal_studies") {
  ...
  if (length(compare_files) > 0L) {
    ...
    message("No portal_studies_*_full.tsv under ", metrics_dir,
            "; falling back to ", merged_path)
...
portal <- load_portal_metrics("paper/metrics/portal_studies")
...
summary_path <- "paper/metrics/benchmarks/bundle_scaling_summary.tsv"
if (file.exists(summary_path)) {
  ...
  message("No summary TSV; falling back to per-rep table at ",
          "paper/metrics/benchmarks/bundle_scaling.tsv")
  scaling <- read_tsv("paper/metrics/benchmarks/bundle_scaling.tsv")
